...
@property (nonatomic, strong) CALayer *firstDot, *secondDot, *thirdDot, *fourthDot;
...
- (void)setupViewLayout:(CGRect)frame
{
    ...
    //Third dot is the one that moves around all four positions clockwise
    _fourthDot = [CALayer layer];
    [_fourthDot setMasksToBounds:YES];
    [_fourthDot setBackgroundColor:[self.color CGColor]];
    [_fourthDot setCornerRadius:_dotRadius];
    [_fourthDot setBounds:CGRectMake(0.0f, 0.0f, _dotRadius*2, _dotRadius*2)];
    _fourthDot.frame = _fourthPoint;
    
    [[self layer] addSublayer:_firstDot];
    [[self layer] addSublayer:_secondDot];
    [[self layer] addSublayer:_thirdDot];
    [[self layer] addSublayer:_fourthDot];
